...
for ttype, name in types.items():
    ...
    # --- Merge ratings ---
    df = df.merge(ratings, on="tconst", how="inner")
    logger.info(f"{len(df):,} {name}")
    ...
    df = df.rename(columns={
        ...
        "genres": "Genres",
        "numVotes": "Votes"
    })[["Title", "Length", "Year", "Rating", "Votes", "Genres", "IMDb Link"]]


    dfs[name] = df  # store in dictionary for later use

# --- Write all sheets to Excel ---
with pd.ExcelWriter("imdb_ratings_noep.xlsx", engine="openpyxl") as writer:
